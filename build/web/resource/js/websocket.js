    ...
    if ("WebSocket" in window) {
        ...
        ws.onmessage = function (event) {
            ...
            if (!window.location.href.includes("/chat")) {
                if (message.content != "[O]pen" && message.content != "[C]lose" && document.getElementById("new-item-message" + newMessage.senderID) == null) {
                    setNewBoxChat(message.senderID)
                }
            } else {
...
function setNewBoxChat(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var html = '<li class="new-item-message" ' + css + ' id="new-item-message' + id + '"> \n\
                                <a href="chat?id=' + id + '"><div class="box-img"><img src="data:image/jpeg;base64,' + data.avatar + '" >\n\
                            <span class="online_icon" id="box-friend-status-onl' + id + '"></span></div></a></li>'
...
function send() {
    ...
    if (listMsg != null && listMsg.length > 0) {
        lastMsg = listMsg[listMsg.length - 1]
